...
Try licence activation and customer sign-in before adding the [SDK](../../../sdk/go/README.md)
to your app. The example checks the `export` feature before producing a sample report.

## Run the example

You need Go 1.27.1 or newer. In the Orbit dashboard, select your application and
...
On Windows, use `-o ../../../orbit-example.exe`, then run `.\orbit-example.exe`
with the same arguments. The default build requires HTTPS. For a local HTTP
service only, add `-tags orbit_local` before `-o`; HTTP is limited to a literal
...
The example's `go.mod` replaces the public SDK module path with the kit's local
`sdk/go` directory so this source ZIP builds before publication. For an app
outside this kit, follow the [Go SDK setup](../../../sdk/go/README.md). Normal
`go get` will work after a public release is tagged using Go's `sdk/go/v...`
submodule tag form.
